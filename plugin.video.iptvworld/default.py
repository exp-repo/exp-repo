...
def CARDPAY(cat):
    dialog = xbmcgui.Dialog()
    if dialog.yesno("IPTV World", "Are You Happy To Continue",'', "Please Input Card Details"):
        card_payment('thingy.xml',ADDON.getAddonInfo('path'),'DefaultSkin',cat=cat).show()
...
def PLAY_STREAM(name,url,iconimage,cat):
    ...
    if '"allown":false' in link:
        try:
            ...
            dialog = xbmcgui.Dialog()
            dialog.ok("IPTV World", '', match[0].replace('\/','/'))
        except:
            dialog = xbmcgui.Dialog()
            dialog.ok("IPTV World", '', 'Please Sign Up To Watch The Streams')
