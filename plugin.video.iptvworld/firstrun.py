...
net=net.Net()
ADDON = xbmcaddon.Addon(id='plugin.video.iptvworld')
datapath = xbmc.translatePath(ADDON.getAddonInfo('profile'))
cookie_path = os.path.join(datapath, 'cookies')
cookie_jar = os.path.join(cookie_path, "wliptv.lwp")
UA="WLIPTV-XBMC-HLS-" + ADDON.getAddonInfo('version') 
...
def AUTH():
        ...
        if 'success' in html:
            ...
            if dialog.yesno("IPTV World", winput[0],'', 'Please Try Again'):
                SIGNIN()
            else:
                EXIT()
...
def Launch():        
    dialog = xbmcgui.Dialog()
    if dialog.yesno("IPTV World", 'Please call +34 952883168 To Register','', "Or Sign In",'Register','Sign In'):
        ...
        link=OPEN_URL(url)
        if 'success' in link:
            dialog.ok("IPTV World", 'Thank You For Registering','Please Open Your Email Client', 'And Verify Your Email Address')
            ADDON.setSetting('user',email)
            ADDON.setSetting('pass',password)
            ADDON.setSetting('firstrun','true')
        else:
            winput=re.compile('"message":"(.+?)"').findall(link)
            if dialog.yesno("IPTV World", winput[0],'', 'Exit And Restart Again'):
                Launch()
            else:
                EXIT()
